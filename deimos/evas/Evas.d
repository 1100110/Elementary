...
immutable EVAS_VERSION_MAJOR = 1;
immutable EVAS_VERSION_MINOR = 2;
...
__gshared Evas_Version* evas_version;
 ...
 * @since 1.1.0
 */
immutable EVAS_CALLBACK_PRIORITY_BEFORE = -100;
 ...
 * @since 1.1.0
 */
immutable EVAS_CALLBACK_PRIORITY_DEFAULT = 0;
 ...
 * @since 1.1.0
 */
immutable EVAS_CALLBACK_PRIORITY_AFTER = 100;
 ...
 * @since 1.1.0
 */
alias short Evas_Callback_Priority;
...
enum Evas_Event_Flags           {
    ...
    EVAS_EVENT_FLAG_ON_HOLD     = (1 << 0),
      ...
      the event should be used for informational purposes and maybe some indications visually,
      but not actually perform anything */
    EVAS_EVENT_FLAG_ON_SCROLL   = (1 << 1)
        /**< This event flag indicates the event occurs while scrolling; for example, DOWN event
          occurs during scrolling; the event should be used for informational purposes and maybe
          some indications visually, but not actually perform anything */
}   /**< Flags for Events */
...
{
    EVAS_COLORSPACE_ARGB8888,   /**< ARGB 32 bits per pixel, high-byte is Alpha, accessed 1 32bit word
                                    at a time */
                                /* these are not currently supported - but planned for the future */
    EVAS_COLORSPACE_YCBCR422P601_PL, /**< YCbCr 4:2:2 Planar, ITU.BT-601 specifications. The data pointed
                                      to is just an array of row pointer, pointing to the Y rows,
                                      then the Cb, then Cr rows */
    EVAS_COLORSPACE_YCBCR422P709_PL, /**< YCbCr 4:2:2 Planar, ITU.BT-709 specifications.
                                        The data pointed to is just an array of row pointer, pointing
                                        to the Y rows, then the Cb, then Cr rows */
    EVAS_COLORSPACE_RGB565_A5P,      /**< 16bit rgb565 + Alpha plane at end - 5 bits of the 8 being
                                        used per alpha byte */
    EVAS_COLORSPACE_GRY8,            /**< 8bit grayscale */
    EVAS_COLORSPACE_YCBCR422601_PL,  /**<  YCbCr 4:2:2, ITU.BT-601 specifications. The data pointed
                                      to is just an array of row pointer, pointing to line
                                      of Y,Cb,Y,Cr bytes */
    EVAS_COLORSPACE_YCBCR420NV12601_PL, /**< YCbCr 4:2:0, ITU.BT-601 specification. The data pointed
                                          to is just an array of row pointer, pointing to the Y rows,
                                          then the Cb,Cr rows. */
    EVAS_COLORSPACE_YCBCR420TM12601_PL, /**< YCbCr 4:2:0, ITU.BT-601 specification. The data pointed
                                          to is just an array of tiled row pointer, pointing to the
                                          Y rows, then the Cb,Cr rows. */
}           /**< Colorspaces for pixel data supported by Evas */
 ...
 */
struct Evas_Object;              /**< An Evas Performance handle */
alias void      Evas_Performance;/**< An opaque type containing information on which modifier keys are registered in an Evas canvas */

struct Evas_Modifier;   /**< An opaque type containing information on which modifier keys are registered in an Evas canvas */
                        /**< An opaque type containing information on which */
struct Evas_Lock;       /** lock keys are registered in an Evas canvas */
struct Evas_Smart;      /**< An Evas Smart Object handle */
...
alias ulong                         Evas_Modifier_Mask;
alias int                           Evas_Coord;
alias int                           Evas_Font_Size;
alias int                           Evas_Angle;

struct Evas_Coord_Rectangle /**< A rectangle in Evas_Coord */
{
...
struct Evas_Coord_Point
{
   Evas_Coord x, y;
}

struct Evas_Coord_Precision_Point
{
   Evas_Coord   x,      y;
   double       xsub,   ysub;
}

struct Evas_Position
{
    Evas_Point          output;
    Evas_Coord_Point    canvas;
}

struct Evas_Precision_Position
{
    Evas_Point                  output;
...
enum Evas_Load_Error
{
    ...
    EVAS_LOAD_ERROR_PERMISSION_DENIED           = 3,/**< Permission denied to an existing file (path) */
...
enum Evas_Pixel_Import_Pixel_Format
{
...
immutable EVAS_NATIVE_SURFACE_VERSION = 2;

enum Evas_Native_Surface_Type
{
...
struct Evas_Native_Surface
{
    ...
    union data {
        ...
        ulong            pixmap; /**< pixmap id to use (Pixmap) */
        }
        struct opengl {
            uint   texture_id;      /**< opengl texture id to use from
                                            glGenTextures() */
            uint   framebuffer_id;  /**< 0 if not a FBO, FBO id
                                        otherwise from glGenFramebuffers() */
            uint   internal_format; /**< same as 'internalFormat' for
                                        glTexImage2D() */
            uint   format;          /**< same as 'format' for glTexImage2D() */
            uint   x, y, w, h;
            /**< region inside the texture to use (image size is assumed as texture size, with 0, 0
            being the top-left and co-ordinates working down to the right and bottom being positive) */
        }
    }
}
 ...
 * @since 1.1.0
 */
immutable EVAS_VIDEO_SURFACE_VERSION = 1;

alias void function(void* data, Evas_Object* obj, const Evas_Video_Surface* surface)
                *Evas_Video_Cb;
alias void function(void* data, Evas_Object* obj, const Evas_Video_Surface* surface,
                Evas_Coord a, Evas_Coord b)
                *Evas_Video_Coord_Cb;

struct Evas_Video_Surface
{
   int Eversion;
...
immutable EVAS_LAYER_MIN        = -32768;/**< bottom-most layer number */
immutable EVAS_LAYER_MAX        = 32767; /**< top-most layer number */

immutable EVAS_COLOR_SPACE_ARGB = 0; /**< Not used for anything */
immutable EVAS_COLOR_SPACE_AHSV = 1; /**< Not used for anything */
immutable EVAS_TEXT_INVALID     = -1;/**< Not used for anything */
immutable EVAS_TEXT_SPECIAL     = -2;/**< Not used for anything */

immutable EVAS_HINT_EXPAND      = 1.0;
/**< Use with evas_object_size_hint_weight_set(), evas_object_size_hint_weight_get(),
  evas_object_size_hint_expand_set(), evas_object_size_hint_expand_get() */

immutable EVAS_HINT_FILL = -1.0; /**< Use with evas_object_size_hint_align_set(),
                                 evas_object_size_hint_align_get(), evas_object_size_hint_fill_set(),
                                 evas_object_size_hint_fill_get() */
// immutable evas_object_size_hint_fill_set = evas_object_size_hint_align_set;
/**< Convenience macro to make it easier to understand that align is also used for fill
  properties (as fill is mutually exclusive to align) */
// immutable evas_object_size_hint_fill_get = evas_object_size_hint_align_get;
/**< Convenience macro to make it easier to understand that align is also used for fill
  properties (as fill is mutually exclusive to align) */
// immutable evas_object_size_hint_expand_set = evas_object_size_hint_weight_set;
/**< Convenience macro to make it easier to understand that weight is also used for expand properties */
// immutable evas_object_size_hint_expand_get = evas_object_size_hint_weight_get;
/**< Convenience macro to make it easier to understand that weight is also used for expand properties */
...
enum Evas_Border_Fill_Mode
{
   ...
   EVAS_BORDER_FILL_DEFAULT = 1,
    /**< Image's center region is to be @b blended with objects
     * underneath it, if it has transparency. This is the default
     * behavior for image objects */
   EVAS_BORDER_FILL_SOLID   = 2
    /**< Image's center region is to be made solid, even if it has
     * transparency on it */
}/**< How an image's center region (the complement to the
        border region) should be rendered by Evas */

enum Evas_Image_Scale_Hint
{
...
enum Evas_Image_Animated_Loop_Hint
{
...
struct Evas_Engine_Info /** Generic engine information. Generic info is useless */
{
   int magic; /**< Magic number */
}

struct Evas_Event_Mouse_Down /** Mouse button press event */
{
   int                  button;
...
struct Evas_Event_Mouse_Up /** Mouse button release event */
{
   int                  button;
...
struct Evas_Event_Mouse_In /** Mouse enter event */
{
   int                  buttons;
        /**< Button pressed mask, Bits set to 1 are buttons currently
...
struct Evas_Event_Mouse_Out /** Mouse leave event */
{
   int                  buttons;
   /**< Button pressed mask, Bits set to 1 are buttons currently pressed
     (bit 0 = mouse button 1, bit 1 = mouse button 2 etc.) */
...
struct Evas_Event_Mouse_Move /** Mouse button down event */
{
   int                  buttons;
   /**< Button pressed mask, Bits set to 1 are buttons currently pressed
     (bit 0 = mouse button 1, bit 1 = mouse button 2 etc.) */
...
struct Evas_Event_Mouse_Wheel /** Wheel event */
{
...
struct Evas_Event_Multi_Down /** Multi button press event */
{
   int                  device;
...
struct Evas_Event_Multi_Up /** Multi button release event */
{
...
struct Evas_Event_Multi_Move /** Multi button down event */
{
...
struct Evas_Event_Key_Down /** Key press event */
{
   ...
   const char*          compose;
   /**< A UTF8 string if this keystroke has modified a string in the middle of being
     composed - this string replaces the previous one */
   uint                 timestamp;
   Evas_Event_Flags     event_flags;
   Evas_Device*         dev;
}

struct Evas_Event_Key_Up /** Key release event */
{
...
struct Evas_Event_Hold /** Hold change event */
{
...
enum Evas_Object_Pointer_Mode
{
   ...
   EVAS_OBJECT_POINTER_MODE_NOGRAB_NO_REPEAT_UPDOWN
       /**< useful on object with "repeat events" enabled, where mouse/touch up and down events
         WONT be repeated to objects and these objects wont be auto-grabbed. @since 1.2 */
} /**< How the mouse pointer should be handled by Evas. */

alias void function(void* data, Evas_Object* obj, void* event_info) *Evas_Smart_Cb;
/**< Evas smart objects' "smart callback" function signature */
alias void function(void* data, Evas* e, void* event_info) *Evas_Event_Cb;
/**< Evas event callback function signature */
alias Eina_Bool function(void* data, Evas* e) *Evas_Object_Event_Post_Cb;

alias void function(void* data, Evas* e, Evas_Object* obj, void* event_info)
                *Evas_Object_Event_Cb;

/**< Evas object event callback function signature */
alias void function(void* target, Evas_Callback_Type type, void* event_info)
 ...
 * @ingroup Evas_Group
 */
alias Eina_Bool evas_async_events_put(const void* target, Evas_Callback_Type type,
                                void* event_info, Evas_Async_Events_Put_Cb func);
 ...
 * @since 1.1.0
 */
void evas_output_framespace_set(Evas* e, Evas_Coord x, Evas_Coord y,
                                Evas_Coord w, Evas_Coord h);
 ...
 * @since 1.1.0
 */
void evas_output_framespace_get(const Evas* e, Evas_Coord* x, Evas_Coord* y,
                                Evas_Coord* w, Evas_Coord* h);
 ...
 * @since 1.1.0
 */
void evas_event_callback_priority_add(Evas* e, Evas_Callback_Type type,
                    Evas_Callback_Priority priority, Evas_Event_Cb func,
                    const void* data);
 ...
 */
void* evas_event_callback_del_full(Evas* e, Evas_Callback_Type type,
                                Evas_Event_Cb func, const void* data);
 ...
 *
 */
void evas_post_event_callback_push(Evas* e, Evas_Object_Event_Post_Cb func,
                                    const void* data);
...
void evas_event_feed_mouse_down(Evas* e, int b, Evas_Button_Flags flags,
  uint timestamp, const void* data);
 ...
 *
 */
void evas_event_feed_mouse_up(Evas* e, int b, Evas_Button_Flags flags, uint timestamp, const void* data);
 ...
 *
 */
void evas_event_feed_mouse_move(Evas* e, int x, int y, uint timestamp, const void* data);
 ...
 *
 */
void evas_event_feed_mouse_in(Evas* e, uint timestamp, const void* data);
 ...
 *
 */
 void function(Evas* e, uint timestamp, const void* data)
            evas_event_feed_mouse_out;

 void function(Evas* e, int d, int x, int y, double rad, double radx,
        double rady, double pres, double ang, double fx, double fy,
        Evas_Button_Flags flags, uint timestamp, const void* data)
 ...
 *
 */
 void evas_event_feed_mouse_cancel(Evas* e, uint timestamp, const void* data);
 ...
 *
 */
 void evas_event_feed_key_down(Evas* e, const char* keyname, const char* key,
    const char* Estring, const char* compose,
    uint timestamp, const void* data);
 ...
 *
 */
 void evas_event_feed_hold(Evas* e, int hold, uint timestamp,
                            const void* data);
 ...
 */
 void evas_event_refeed_event(Evas* e, void* event_copy,
 ...
 * @ingroup Evas_Font_Path_Group
 */
 Eina_List* evas_font_path_list(const Evas* e);
 ...
 * @endcode
 */
 Eina_List* evas_object_clipees_get(const Evas_Object* obj);
 ...
 * @see evas_object_key_ungrab
 */
 Eina_Bool evas_object_focus_get(const Evas_Object* job);
 ...
 * See the full @ref Example_Evas_Events "example".
 */
 char* evas_object_name_get(const Evas_Object* obj);
 ...
 * @ingroup Evas_Object_Group_Basic
 */
 void evas_object_geometry_get(const Evas_Object* obj, Evas_Coord* x,
                        Evas_Coord* y, Evas_Coord* w, Evas_Coord* h);
 ...
 * @ingroup Evas_Object_Group_Basic
 */
 void evas_object_color_set(Evas_Object* obj, int r, int g,
                            int b, int a);
 ...
 * @ingroup Evas_Object_Group_Basic
 */
 void evas_object_color_get(const Evas_Object* obj, int* r, int* g,
                                int* b, int* a);
 ...
 * See the full @ref Example_Evas_Object_Manipulation "example".
 */
 char* evas_object_type_get(const Evas_Object* obj);
 ...
 *
 */
void function(Evas_Object* obj, Evas_Callback_Type type,
 ...
 * @since 1.1.0
 */
 void evas_object_event_callback_priority_add
        (Evas_Object* obj, Evas_Callback_Type type,
        Evas_Callback_Priority priority, Evas_Object_Event_Cb func,
        const void* data);
 ...
 */
 void function(Evas_Object* obj, Evas_Callback_Type type,
        Evas_Object_Event_Cb func) *evas_object_event_callback_del;
 ...
 */
 void evas_object_map_source_set(Evas_Object* obj, Evas_Object* src);
 ...
 */
 Evas_Object* evas_object_map_source_get(const Evas_Object *obj);
 ...
 */
 Evas_Map* evas_object_map_get(const Evas_Object* obj);
 ...
 */
 void evas_map_util_points_populate_from_object_full(Evas_Map* m,
                                const Evas_Object* obj, Evas_Coord z);
 ...
 * @see evas_map_point_image_uv_set()
 */
 void evas_map_util_points_populate_from_geometry
            (Evas_Map* m, Evas_Coord x, Evas_Coord y,
 ...
 */
 void evas_map_util_points_color_set(Evas_Map* m, int r, int g,
                                        int b, int a);
 ...
 */
 void evas_map_util_rotate(Evas_Map* m, double degrees, Evas_Coord cx,
                            Evas_Coord cy);
 ...
 */
 void evas_map_util_zoom(Evas_Map* m, double zoomx, double zoomy,
 ...
 */
 void evas_map_util_3d_rotate(Evas_Map* m, double dx, double dy,
                double dz, Evas_Coord cx, Evas_Coord cy, Evas_Coord cz);
 ...
 */
 void evas_map_util_3d_lighting(Evas_Map* m, Evas_Coord lx,
                                Evas_Coord ly, Evas_Coord lz, int lr,
                                int lg, int lb, int ar, int ag, int ab);
 ...
 * @see evas_map_util_points_populate_from_object()
 */
 void function(Evas_Map* m, int idx, Evas_Coord x, Evas_Coord y, Evas_Coord z)
 ...
 */
 void function(const Evas_Map* m, int idx, Evas_Coord* x, Evas_Coord* y,
        Evas_Coord* z) evas_map_point_coord_get;
 ...
 * @see evas_map_util_points_populate_from_object()
 */
 void  function(Evas_Map* m, int idx, double u, double v)
        evas_map_point_image_uv_set;
 ...
 */
 void function(const Evas_Map* m, int idx, double* u, double* v)
 ...
 * @see evas_object_map_set()
 */
 void function(Evas_Map* m, int idx, int r, int g, int b, int a)
 ...
 * @see evas_object_map_set()
 */
 void function(const Evas_Map* m, int idx, int* r, int* g, int* b, int* a)
 ...
 */
 void function(const Evas_Object* obj, Evas_Coord* w, Evas_Coord* h)
 ...
 */
 void function(Evas_Object* obj, Evas_Coord w, Evas_Coord h)
 ...
 */
 void function(const Evas_Object* obj, Evas_Aspect_Control* aspect,
        Evas_Coord* w, Evas_Coord* h) evas_object_size_hint_aspect_get;
 ...
 */
 void function(Evas_Object* obj, Evas_Aspect_Control aspect, Evas_Coord w,
        Evas_Coord h) evas_object_size_hint_aspect_set;
 ...
 */
 void  function(const Evas_Object* obj, double* x, double* y) evas_object_size_hint_align_get;
 ...
 */
 void function(Evas_Object* obj, double x, double y) evas_object_size_hint_align_set;
 ...
 */
 void function(const Evas_Object* obj, double* x, double* y) evas_object_size_hint_weight_get;
 ...
 */
 void function(Evas_Object* obj, double x, double y) evas_object_size_hint_weight_set;
 ...
 * @see evas_object_size_hint_padding_set()
 */
 void function(const Evas_Object* obj, Evas_Coord* l,
 ...
 */
 void function(Evas_Object* obj, Evas_Coord l, Evas_Coord r,
            Evas_Coord t, Evas_Coord b) evas_object_size_hint_padding_set;
 ...
 * @endcode
 */
 void function(Evas_Object* obj, const char* key, const void* data) evas_object_data_set;
 ...
 * @endcode
 */
 void function(const Evas_Object* obj, const char* key) *evas_object_data_get;
 ...
 * @endcode
 */
 void function(Evas_Object* obj, const char* key) *evas_object_data_del;
 ...
 */
 Evas_Object* evas_object_top_at_xy_get(const Evas* e, Evas_Coord x,
            Evas_Coord y, Eina_Bool include_pass_events_objects,
...
alias void function(void* data, Evas_Object* obj) *Evas_Object_Intercept_Show_Cb;
alias void function(void* data, Evas_Object* obj) *Evas_Object_Intercept_Hide_Cb;
alias void function(void* data, Evas_Object* obj, Evas_Coord x, Evas_Coord y) *Evas_Object_Intercept_Move_Cb;
alias void function(void* data, Evas_Object* obj, Evas_Coord w, Evas_Coord h) *Evas_Object_Intercept_Resize_Cb;
alias void function(void* data, Evas_Object* obj) *Evas_Object_Intercept_Raise_Cb;
alias void function(void* data, Evas_Object* obj) *Evas_Object_Intercept_Lower_Cb;
alias void function(void* data, Evas_Object* obj, Evas_Object* above) *Evas_Object_Intercept_Stack_Above_Cb;
alias void function(void* data, Evas_Object* obj, Evas_Object* above) *Evas_Object_Intercept_Stack_Below_Cb;
alias void function(void* data, Evas_Object* obj, int l) *Evas_Object_Intercept_Layer_Set_Cb;
alias void function(void* data, Evas_Object* obj, int r, int g, int b, int a) *Evas_Object_Intercept_Color_Set_Cb;
alias void function(void* data, Evas_Object* obj, Evas_Object* clip) *Evas_Object_Intercept_Clip_Set_Cb;
alias void function(void* data, Evas_Object* obj) *Evas_Object_Intercept_Clip_Unset_Cb;
 ...
 *
 */
alias void  function(Evas_Object* obj, Evas_Object_Intercept_Show_Cb func, const void* data)
 ...
 *
 */
alias void function(Evas_Object* obj, Evas_Object_Intercept_Show_Cb func)
 ...
 *
 */
alias void function(Evas_Object* obj, Evas_Object_Intercept_Hide_Cb func, const void* data)
 ...
 *
 */
alias void function(Evas_Object* obj, Evas_Object_Intercept_Hide_Cb func)
 ...
 */
 void evas_object_intercept_move_callback_add(Evas_Object* obj,
        Evas_Object_Intercept_Move_Cb func, const void* data);
 ...
 *
 */
void function(Evas_Object* obj, Evas_Object_Intercept_Move_Cb func)
        *evas_object_intercept_move_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Resize_Cb func,
        const void* data) evas_object_intercept_resize_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Resize_Cb func)
        *evas_object_intercept_resize_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Raise_Cb func,
        const void* data) evas_object_intercept_raise_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Raise_Cb func)
        *evas_object_intercept_raise_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Lower_Cb func,
        const void* data) evas_object_intercept_lower_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Lower_Cb func)
        *evas_object_intercept_lower_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Stack_Above_Cb func,
        const void* data) evas_object_intercept_stack_above_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Stack_Above_Cb func)
        *evas_object_intercept_stack_above_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Stack_Below_Cb func,
        const void* data) evas_object_intercept_stack_below_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Stack_Below_Cb func)
        *evas_object_intercept_stack_below_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Layer_Set_Cb func,
        const void* data) evas_object_intercept_layer_set_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Layer_Set_Cb func)
        *evas_object_intercept_layer_set_callback_del;

void  function(Evas_Object* obj, Evas_Object_Intercept_Color_Set_Cb func,
        const void* data) evas_object_intercept_color_set_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Color_Set_Cb func)
        *evas_object_intercept_color_set_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Clip_Set_Cb func,
        const void* data) evas_object_intercept_clip_set_callback_add;

void function(Evas_Object* obj, Evas_Object_Intercept_Clip_Set_Cb func)
        *evas_object_intercept_clip_set_callback_del;

void function(Evas_Object* obj, Evas_Object_Intercept_Clip_Unset_Cb func,
        const void* data) evas_object_intercept_clip_unset_callback_add;
 ...
 *   region. All pixels are 32-bit uint's with the high-byte
 ...
 * @{
 */
alias void function(void* data, Evas_Object* o) *Evas_Object_Image_Pixels_Get_Cb;
 ...
 *
 * @note If youintend to @b display an image somehow in a GUI,
 ...
 */
 void evas_object_image_memfile_set(Evas_Object* obj, void* data,
                                    int size, char* format, char* key);
 ...
 * @endcode
 */
 void  evas_object_image_file_set(Evas_Object* obj, const char* file,
                                    const char* key);
 ...
 * interested in: they'll be ignored by the function.
 */
 void evas_object_image_file_get(const Evas_Object* obj, const char** file,
                    const char** key);
 ...
 */
 void evas_object_image_border_set(Evas_Object* obj, int l, int r, int t,
                                        int b);
 ...
 */
 void  evas_object_image_border_get(const Evas_Object* obj, int* l, int* r,
                                        int* t, int* b);
 ...
 */
 Evas_Border_Fill_Mode evas_object_image_border_center_fill_get
            (const Evas_Object* obj);
 ...
 * given way.
 */
 void  evas_object_image_fill_set (Evas_Object* obj, Evas_Coord x,
                                Evas_Coord y, Evas_Coord w, Evas_Coord h);
 ...
 * See @ref evas_object_image_fill_set() for more details.
 */
 void  evas_object_image_fill_get(const Evas_Object* obj, Evas_Coord* x,
                            Evas_Coord* y, Evas_Coord* w, Evas_Coord* h);
 ...
 */
 void  evas_object_image_data_update_add(Evas_Object* obj, int x, int y,
                                            int w, int h);
 ...
 */
 void evas_object_image_load_size_get(const Evas_Object* obj, int* w,
                    int* h);
 ...
 */
 void  evas_object_image_load_region_set(Evas_Object* obj, int x, int y,
                int w, int h);
 ...
 * @see evas_object_image_load_region_get()
 */
 void evas_object_image_load_region_get(const Evas_Object* obj, int* x,
                int* y, int* w, int* h);
 ...
 *
 */
 Evas_Native_Surface* evas_object_image_native_surface_get
            (const Evas_Object* obj);
 ...
 *
 */
 Evas_Video_Surface* evas_object_image_video_surface_get(const Evas_Object* obj);
 ...
 */
 char* evas_object_text_font_source_get(const Evas_Object* obj);
 ...
 * @see evas_object_text_text_set()
 */
 char* evas_object_text_text_get(const Evas_Object* obj);
 ...
 * @since 1.1.0
 */
 char* evas_object_text_bidi_delimiters_get(const Evas_Object* obj);

Evas_Coord function(const Evas_Object* obj) evas_object_text_ascent_get        ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_descent_get       ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_max_ascent_get    ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_max_descent_get   ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_horiz_advance_get ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_vert_advance_get  ;//EINA_WARN_UNUSED_RESULT ;
Evas_Coord function(const Evas_Object* obj) evas_object_text_inset_get         ;//EINA_WARN_UNUSED_RESULT ;
 ...
 */
 Eina_Bool evas_object_text_char_pos_get(const Evas_Object* obj, int pos,
 ...
 int evas_object_text_char_coords_get(const Evas_Object* obj, Evas_Coord x,
            Evas_Coord y, Evas_Coord* cx, Evas_Coord* cy, Evas_Coord* cw,
            Evas_Coord* ch);
 ...
 */
 int evas_object_text_last_up_to_pos(const Evas_Object* obj, Evas_Coord x,
                                        Evas_Coord y);
 ...
 */
 void  evas_object_text_shadow_color_set(Evas_Object* obj, int r, int g,
                                            int b, int a);
 ...
 */
 void  evas_object_text_shadow_color_get(const Evas_Object* obj, int* r,
                                            int* g, int* b, int* a);
 ...
 */
 void  evas_object_text_glow_color_set(Evas_Object* obj, int r, int g,
                                            int b, int a) ;
 ...
 */
 void  evas_object_text_glow_color_get(const Evas_Object* obj, int* r,
                                            int* g, int* b, int* a);
 ...
 */
 void  evas_object_text_glow2_color_set(Evas_Object* obj, int r, int g,
                                            int b, int a);
 ...
 */
 void  evas_object_text_glow2_color_get(const Evas_Object* obj, int* r,
                                            int* g, int* b, int* a);
 ...
 */
 void  evas_object_text_outline_color_set(Evas_Object* obj, int r,
                                            int g, int b, int a);
 ...
 *
 */
 void evas_object_text_style_pad_get(const Evas_Object* obj, int* l, int* r,
                                        int* t, int* b);
 ...
 */
   ...
   struct Evas_Textblock_Style;
   struct Evas_Textblock_Cursor;
   /**
    * @typedef Evas_Object_Textblock_Node_Format
    * A format node.
    */
   struct Evas_Object_Textblock_Node_Format;
    ...
    enum Evas_Textblock_Cursor_Type
    {
 ...
 */
 char* evas_textblock_escape_string_get(const char* escape);
 ...
 */
 char* evas_textblock_string_escape_get(const char* string, int* len_ret);
 ...
 */
 char* evas_textblock_escape_string_range_get
            (const char* escape_start, const char* escape_end);
 ...
 */
 char* evas_textblock_style_get(const Evas_Textblock_Style* ts);
 ...
 * @since 1.1.0
 */
 void evas_object_textblock_valign_set(Evas_Object* obj, double Ealign);
 ...
 * @since 1.1.0
 */
 char* evas_object_textblock_bidi_delimiters_get(const Evas_Object* obj);
 ...
 */
 char* evas_object_textblock_text_markup_get(const Evas_Object* obj);
 ...
 */
 Evas_Textblock_Cursor* evas_object_textblock_cursor_get
            (const Evas_Object* obj);
 ...
 */
 Evas_Textblock_Cursor* evas_object_textblock_cursor_new
            (const Evas_Object* obj);
 ...
 */
 Eina_List* evas_textblock_node_format_list_get
            (const Evas_Object* obj, const char* anchor);
 ...
 */
 Evas_Object_Textblock_Node_Format* evas_textblock_node_format_first_get
            (const Evas_Object* obj);
 ...
 * @return Returns the first format node, may be null if there are none.
 */
 Evas_Object_Textblock_Node_Format* evas_textblock_node_format_last_get
            (const Evas_Object* obj);
 ...
 */
 Evas_Object_Textblock_Node_Format* evas_textblock_node_format_next_get
 ...
 */
 Evas_Object_Textblock_Node_Format* evas_textblock_node_format_prev_get
            (const Evas_Object_Textblock_Node_Format* n);
 ...
 */
 Evas_Object_Textblock_Node_Format* evas_textblock_cursor_format_get
 ...
 */
 char* evas_textblock_node_format_text_get
 ...
 */
 Eina_Bool evas_textblock_cursor_format_is_visible_get
            (const Evas_Textblock_Cursor* cur);
 ...
 */
 char* evas_textblock_cursor_paragraph_text_get
        (const Evas_Textblock_Cursor* cur);
 ...
 */
 int evas_textblock_cursor_paragraph_text_length_get
        (const Evas_Textblock_Cursor* cur);
 ...
 * @since 1.1.0
 */
 Eina_Bool evas_textblock_cursor_visible_range_get
 ...
 */
 Eina_List* evas_textblock_cursor_range_formats_get
        (const Evas_Textblock_Cursor* cur1,
            const Evas_Textblock_Cursor* cur2);
 ...
 */
 char* evas_textblock_cursor_range_text_get(const Evas_Textblock_Cursor* cur1,
        const Evas_Textblock_Cursor* cur2, Evas_Textblock_Text_Type format);//EINA_WARN_UNUSED_RESULT EINA_ARG_NONNULL(1, 2);
...
Eina_Bool evas_textblock_cursor_format_item_geometry_get
        (const Evas_Textblock_Cursor* cur, Evas_Coord* cx,
 ...
 */
 Eina_Bool evas_textblock_cursor_eol_get(const Evas_Textblock_Cursor* cur);//EINA_WARN_UNUSED_RESULT ;
 ...
 */
 Eina_Bool evas_object_textblock_line_number_geometry_get(const Evas_Object* obj,
        int line, Evas_Coord* cx, Evas_Coord* cy, Evas_Coord* cw,
        Evas_Coord* ch);
 ...
 * @ingroup Evas_Smart_Group
 */
immutable EVAS_SMART_CLASS_VERSION = 4;
/**
...
struct Evas_Smart_Class
{
   ...
   int         Eversion;
   void  function(Evas_Object* o) *add; /**< code to be run when adding object to a canvas */
   void  function(Evas_Object* o) *del; /**< code to be run when removing object to a canvas */
   void  function(Evas_Object* o, Evas_Coord x, Evas_Coord y) *move; /**< code to be run when moving object on a canvas */
   void  function(Evas_Object* o, Evas_Coord w, Evas_Coord h) *resize; /**< code to be run when resizing object on a canvas */
   void  function(Evas_Object* o) *show; /**< code to be run when showing object on a canvas */
   void  function(Evas_Object* o) *hide; /**< code to be run when hiding object on a canvas */
   void  function(Evas_Object* o, int r, int g, int b, int a) *color_set; /**< code to be run when setting color of object on a canvas */
   void  function(Evas_Object* o, Evas_Object* clip) *clip_set; /**< code to be run when setting clipper of object on a canvas */
   void  function(Evas_Object* o) *clip_unset; /**< code to be run when unsetting clipper of object on a canvas */
   void  function(Evas_Object* o) *calculate; /**< code to be run when object has rendering updates on a canvas */
   void  function(Evas_Object* o, Evas_Object* child) *member_add; /**< code to be run when child member is added to object */
   void  function(Evas_Object* o, Evas_Object* child) *member_del; /**< code to be run when child member is removed from object */
 ...
 */
 Evas_Smart* evas_smart_class_new(const Evas_Smart_Class* sc) ;//EINA_WARN_UNUSED_RESULT  EINA_MALLOC;
 ...
 */
 Evas_Smart_Class* evas_smart_class_get(const Evas_Smart* s) ;//EINA_WARN_UNUSED_RESULT ;
 ...
 */
 Evas_Smart_Cb_Description**
    evas_smart_callbacks_descriptions_get(const Evas_Smart* s,
                                            uint* count);
 ...
 */
 Evas_Smart_Cb_Description*
    evas_smart_callback_description_find(const Evas_Smart* s,
        const char* name);
 ...
 */
 Eina_Bool evas_smart_class_inherit_full(Evas_Smart_Class* sc,
    const Evas_Smart_Class* parent_sc, uint parent_sc_size);// EINA_ARG_NONNULL(1, 2);
 ...
 */
 Evas_Object* evas_object_smart_add(Evas* e, Evas_Smart* s);//EINA_WARN_UNUSED_RESULT EINA_ARG_NONNULL(1, 2) EINA_MALLOC;
 ...
 * @ingroup Evas_Smart_Object_Group
 */
 Evas_Smart* evas_object_smart_smart_get(const Evas_Object* obj) ;//EINA_WARN_UNUSED_RESULT ;
 ...
 * @ingroup Evas_Smart_Object_Group
 */
 void* evas_object_smart_data_get(const Evas_Object* obj);//EINA_WARN_UNUSED_RESULT ;
 ...
 */
 void evas_object_smart_callback_priority_add(Evas_Object* obj,
    const char* event, Evas_Callback_Priority priority, Evas_Smart_Cb func,
    const void* data);
 ...
 */
 void* evas_object_smart_callback_del(Evas_Object* obj,
    const char* event, Evas_Smart_Cb func);
 ...
 */
 void* evas_object_smart_callback_del_full(Evas_Object* obj,
    const char* event, Evas_Smart_Cb func, const void* data);
...
void evas_object_smart_callbacks_descriptions_get(const Evas_Object* obj,
 const Evas_Smart_Cb_Description*** class_descriptions, uint* class_count,
 ...
 */
 void evas_object_smart_callback_description_find(const Evas_Object* obj,
    const char* name, const Evas_Smart_Cb_Description** class_description,
 ...
 * @ingroup Evas_Smart_Object_Group
 */
 Eina_Bool evas_object_smart_need_recalculate_get(const Evas_Object* obj);//EINA_WARN_UNUSED_RESULT ;
...
alias void function(Evas_Object* o, Evas_Object_Box_Data* priv,
                void* user_data) *Evas_Object_Box_Layout;
/**
 ...
 * @ingroup Evas_Object_Box
 */
immutable EVAS_OBJECT_BOX_API_VERSION = 1;
 ...
 *///TODO
  ...
  //      int                     Eversion; /**< Version of this smart class definition */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child)                               *append           ;/**< Smart function to append child elements in boxes */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child)                               *prepend          ;/**< Smart function to prepend child elements in boxes */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child, const Evas_Object* reference) *insert_before    ;/**< Smart function to insert a child element before another in boxes */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child, const Evas_Object* reference) *insert_after     ;/**< Smart function to insert a child element after another in boxes */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child, uint pos)                     *insert_at        ;/**< Smart function to insert a child element at a given position on boxes */
  //      Evas_Object*            (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child)                               *remove           ;/**< Smart function to remove a child element from boxes */
  //      Evas_Object*            (Evas_Object* o, Evas_Object_Box_Data* priv, uint pos)                                         *remove_at        ;/**< Smart function to remove a child element from boxes, by its position */
  //      Eina_Bool               (Evas_Object* o, Evas_Object_Box_Option* opt, int property, va_list args)                      *property_set     ;/**< Smart function to set a custom property on a box child */
  //      Eina_Bool               (Evas_Object* o, Evas_Object_Box_Option* opt, int property, va_list args)                      *property_get     ;/**< Smart function to retrieve a custom property from a box child */
  //      const char*             (Evas_Object* o, int property)                                                                 *property_name_get;/**< Smart function to get the name of a custom property of box children */
  //      int                     (Evas_Object* o, const char* name)                                                             *property_id_get  ;/**< Smart function to get the numerical ID of a custom property of box children */
  //      Evas_Object_Box_Option* (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object* child)                               *option_new       ;/**< Smart function to create a new box option struct */
  //      void                    (Evas_Object* o, Evas_Object_Box_Data* priv, Evas_Object_Box_Option* opt)                      *option_free      ;/**< Smart function to delete a box option struct */
  // }
...
//        void*                        data;
//        void                         function(void* data) *free_data;
//    }
//    Eina_Bool                        layouting : 1;//TODO
 ...
 * parent class.
 */
 Evas_Object_Box_Api* evas_object_box_smart_class_get();
 ...
 */
 void evas_object_box_layout_set(Evas_Object* o,
            Evas_Object_Box_Layout cb, const void* data,
            void function(void* data) *free_data);
 ...
 */
 void evas_object_box_layout_horizontal(Evas_Object* o,
        Evas_Object_Box_Data* priv, void* data);// EINA_ARG_NONNULL(1, 2);
 ...
 */
 void evas_object_box_align_set(Evas_Object* o, double horizontal,
                                    double vertical);
 ...
 */
 void evas_object_box_padding_set(Evas_Object* o, Evas_Coord horizontal,
                                    Evas_Coord vertical);
 ...
 */
 Evas_Object_Box_Option* evas_object_box_insert_before(Evas_Object* o,
    Evas_Object* child, const Evas_Object* reference);// EINA_ARG_NONNULL(1, 2, 3);
 ...
 */
 Evas_Object_Box_Option* evas_object_box_insert_at(Evas_Object* o, Evas_Object *child, uint pos);// EINA_ARG_NONNULL(1, 2);
 ...
 * smart function.
 */
 Eina_Bool evas_object_box_remove(Evas_Object* o, Evas_Object* child);// EINA_ARG_NONNULL(1, 2);
 ...
 */
 Eina_Bool evas_object_box_remove_at(Evas_Object* o, uint pos);
 ...
 */
 Eina_Iterator* evas_object_box_iterator_new(const Evas_Object* o) ;//EINA_WARN_UNUSED_RESULT  EINA_MALLOC;
 ...
 */
 Eina_Accessor* evas_object_box_accessor_new(const Evas_Object* o) ;//EINA_WARN_UNUSED_RESULT  EINA_MALLOC;
 ...
 */
 char* evas_object_box_option_property_name_get(Evas_Object* o,
                            int property);
 ...
 * box, which is originally set to @c NULL.
 */
 int evas_object_box_option_property_id_get(Evas_Object* o,
                        const char* name);
 ...
 */
 Eina_Bool evas_object_box_option_property_set(Evas_Object* o,
                Evas_Object_Box_Option* opt, int property, ...);
 ...
 */
 Eina_Bool evas_object_box_option_property_vset(Evas_Object* o,
            Evas_Object_Box_Option* opt, int property, va_list args);
/**
 ...
 * previous arguments.
 */
 Eina_Bool evas_object_box_option_property_get(Evas_Object* o,
            Evas_Object_Box_Option* opt, int property, ...);
 ...
 * more details.
 */
 Eina_Bool evas_object_box_option_property_vget(Evas_Object* o,
            Evas_Object_Box_Option* opt, int property, va_list args);// EINA_ARG_NONNULL(1, 2);
 ...
 */
 Evas_Object_Table_Homogeneous_Mode evas_object_table_homogeneous_get
            (const Evas_Object* o);
/**
 ...
 */
 void evas_object_table_align_set(Evas_Object* o, double horizontal,
                                    double vertical);
 ...
 * @since 1.1.0
 */
 Eina_Bool evas_object_table_pack_get(Evas_Object* o, Evas_Object* child, ushort* col, ushort* row, ushort* colspan, ushort* rowspan);
 ...
 * @return 1 on success, 0 on failure.
 */
 Eina_Bool evas_object_table_pack(Evas_Object* o, Evas_Object* child, ushort col, ushort row, ushort colspan, ushort rowspan);// EINA_ARG_NONNULL(1, 2);
 ...
 */
 Evas_Object* evas_object_table_child_get(const Evas_Object* o, ushort col, ushort row);
 ...
 * @since 1.1.0
 */
Eina_Bool evas_object_grid_pack_get(Evas_Object* o, Evas_Object* child,
                                    int* x, int* y, int* w, int* h);
 ...
 */
   struct Evas_Cserve_Image_Cache;

/**
 * Statistics about the server that shares cached bitmaps.
 * @ingroup Evas_Cserve
 */
struct Evas_Cserve_Stats
{
 ...
 * @ingroup Evas_Cserve
 */
Eina_Bool evas_cserve_want_get();
 ...
 * @ingroup Evas_Cserve
 */
Eina_Bool evas_cserve_connected_get();
 ...
 * @ingroup Evas_Utils
 */
char* evas_load_error_str(Evas_Load_Error error);
 ...
 * @ingroup Evas_Utils
 **/
void evas_color_hsv_to_rgb(float h, float s, float v, int* r,
                            int* g, int* b);
 ...
 * @ingroup Evas_Utils
 **/
void evas_color_rgb_to_hsv(int r, int g, int b, float* h,
                                float *s, float* v);
 ...
 */
Evas_Modifier* evas_key_modifier_get(const Evas* e);
 ...
 */
uint evas_touch_point_list_count(Evas* e);
 ...
 * @see evas_touch_point_list_nth_state_get()
 */
void evas_touch_point_list_nth_xy_get(Evas* e, uint n, Evas_Coord* x,
                                        Evas_Coord* y);
 ...
 * @see evas_touch_point_list_nth_state_get()
 */
int evas_touch_point_list_nth_id_get(Evas* e, uint n);
 ...
 * @see evas_touch_point_list_nth_id_get()
 */
Evas_Touch_Point_State evas_touch_point_list_nth_state_get(Evas* e, uint n);
